...
import {
  ComparisonTable, CountdownTimer, FeatureHighlights,
  HowItWorks, LeadMagnetGate, MarketingHeroBlock, PromoPopup,
  StatsCounter, TestimonialsCarousel, TrustBadges,
...
describe("Marketing components", () => {
  ...
  it("FeatureHighlights renders", () => {
    const { container } = wrap(
      <FeatureHighlights items={[{ icon: "star", title: "Fast", description: "Blazing" }]} />
    );
    expect(container.firstChild).toBeTruthy();
  });
  ...
  it("TrustBadges renders", () => {
    const { container } = wrap(
      <TrustBadges badges={[{ label: "Secure", icon: "shield" }]} />
    );
    expect(container.firstChild).toBeTruthy();
  });
