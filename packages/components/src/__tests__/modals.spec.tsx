...
import {
  ...
  LoadingOverlay, Tooltip, ToastBanners, CookieNotice,
  WelcomePopup,
...
describe("Modal components", () => {
  it("OverlayBase renders when open", () => {
    const { container } = wrap(
      <OverlayBase isOpen onClose={noop}><span>content</span></OverlayBase>
    );
  ...
  it("GenericModal renders", () => {
    const { container } = wrap(
      <GenericModal isOpen title="Test" onClose={noop}><p>Body</p></GenericModal>
    );
    expect(container.firstChild).toBeTruthy();
  });
  ...
  it("Tooltip renders", () => {
    const { container } = wrap(
      <Tooltip text="Hint"><button>Hover</button></Tooltip>
    );
    expect(container.firstChild).toBeTruthy();
  });
  ...
  it("CookieNotice renders", () => {
    const { container } = wrap(
      <CookieNotice isOpen onAccept={noop} onDecline={noop} />
    );
    expect(container.firstChild).toBeTruthy();
  });
