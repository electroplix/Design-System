...
describe("Core – provider", () => {
  it("renders children inside ElectroplixProvider", () => {
    const { getByText } = render(
      <ElectroplixProvider><span>Hello</span></ElectroplixProvider>
    );
...
describe("Core – Icon", () => {
  it("renders an SVG element for a known icon name", () => {
    const { container } = render(
      <TestWrapper><Icon name="check" size={20} /></TestWrapper>
    );
  ...
  it("renders nothing for an unknown icon name", () => {
    const { container } = render(
      <TestWrapper><Icon name="nonexistent-icon-xyz" size={20} /></TestWrapper>
    );
