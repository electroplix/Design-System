...
import {
  MediaShell, ResponsiveVideo, AudioEmbed, AvatarProfile,
  IconGrid, ImageGallery, LightboxGallery, MapEmbed, MasonryGrid,
  PolaroidImage, LottieOrSVG, ImageCropperUploader,
...
describe("Media components", () => {
  it("MediaShell renders", () => {
    const { container } = wrap(<MediaShell><span>content</span></MediaShell>);
  ...
  it("IconGrid renders", () => {
    const { container } = wrap(<IconGrid icons={[{ name: "star", label: "Star" }]} />);
    expect(container.firstChild).toBeTruthy();
  });

  it("ImageGallery renders", () => {
    const { container } = wrap(
      <ImageGallery items={[{ src: "img.jpg", alt: "test" }]} />
    );
    expect(container.firstChild).toBeTruthy();
  });
  ...
  it("LightboxGallery renders", () => {
    const { container } = wrap(
      <LightboxGallery items={[{ src: "img.jpg", alt: "test" }]} />
    );
    expect(container.firstChild).toBeTruthy();
  });
  ...
  it("MasonryGrid renders", () => {
    const { container } = wrap(
      <MasonryGrid items={[{ src: "img.jpg", alt: "test" }]} />
    );
    expect(container.firstChild).toBeTruthy();
  });
  ...
  it("MapEmbed renders with coordinates", () => {
    const { container } = wrap(<MapEmbed lat={40.7128} lng={-74.006} zoom={12} provider="openstreetmap" />);
    expect(container.firstChild).toBeTruthy();
  });
});
