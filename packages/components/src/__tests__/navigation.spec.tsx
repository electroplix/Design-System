...
import {
  AnchorLinks, Breadcrumbs, Footer, LanguageSelector, MegaMenu,
...
describe("Navigation components", () => {
  ...
  it("LanguageSelector renders", () => {
    const { container } = wrap(<LanguageSelector languages={[{ code: "en", label: "English" }]} current="en" />);
    expect(container.firstChild).toBeTruthy();
  });

  it("MegaMenu renders", () => {
    const { container } = wrap(<MegaMenu groups={[{ label: "Group", items: [{ label: "Item", href: "#" }] }]} />);
    expect(container.firstChild).toBeTruthy();
  });

  it("Pagination renders", () => {
    const { container } = wrap(<Pagination currentPage={1} totalPages={5} onPageChange={() => {}} />);
    expect(container.firstChild).toBeTruthy();
  });
  ...
  it("SidebarMenu renders", () => {
    const { container } = wrap(<SidebarMenu items={[{ id: "1", label: "Dashboard", href: "#" }]} />);
    expect(container.firstChild).toBeTruthy();
  });

  it("SideDrawerNav renders when open", () => {
    const { container } = wrap(<SideDrawerNav isOpen={true} onClose={() => {}} items={[{ id: "1", label: "Nav", href: "#" }]} />);
    expect(container.firstChild).toBeTruthy();
  });
  ...
  it("Tabs renders", () => {
    const { container } = wrap(<Tabs tabs={[{ id: "1", label: "Tab 1", content: <span>Content</span> }]} />);
    expect(container.firstChild).toBeTruthy();
  });
