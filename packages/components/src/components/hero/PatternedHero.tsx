...
function usePattern(kind: PatternKind, accent: string, intensity: number) {
  return useMemo(() => {
    if (kind === 'dots') {
      return `radial-gradient(circle, ${accent}${Math.round(intensity * 18)
        .toString(16)
    ...
    if (kind === 'grid') {
      return `linear-gradient(${accent}${Math.round(intensity * 16)
        .toString(16)
        .padStart(2, '0')} 1px, transparent 1px),
      linear-gradient(90deg, ${accent}${Math.round(intensity * 16)
        .toString(16)
        .padStart(2, '0')} 1px, transparent 1px)`;
    }
        ...
        >
          <button
            ...
            style={{
              ...
              boxShadow: hover ? '0 8px 24px rgba(9,9,11,0.12)' : '0 1px 2px rgba(9,9,11,0.08)',
            }}
          >
            {ctaLabel}
